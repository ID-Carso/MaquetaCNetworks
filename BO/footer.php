<footer>
    <div class="o-footer-section">
        <div class="row no-gutters terms-conditions-mobile">
            <div class="col-10 col-md-10 mx-auto aviso-border">
                <div class="col-12 col-md-10 m-auto">
                    <div class="logo-footer">
                        <img src="../images/home/claro-grey.svg" alt="" class="claro-grey-img">
                    </div>
                    <p class="text-footer-regular">@2019 Claro tv. Todos los derechos reservados</p>
                    <div class="d-flex justify-content-between">
                        <div>
                            <a href="terminos.html">
                                <p class="aviso-priv-blue">Términos y condiciones</p>
                            </a>
                        </div>
                        <div>
                            <a href="#">
                                <p class="aviso-priv-blue">Aviso de privacidad</p>
                            </a>
                        </div>
                    </div>
                </div>
            </div>
            <div class="footer-companies col-12">
                <div class="d-flex padding-bottom-footer">
                    <div class="col">
                        <img class="d-block m-auto footer-img" src="../images/home/fast-icon.svg" alt="">
                    </div>
                    <div class="col">
                        <img class="d-block m-auto footer-img" src="../images/home/claro-musica-icon.svg" alt="">
                    </div>
                    <div class="col">
                        <img class="d-block m-auto footer-img" src="../images/home/claro-video-icon.svg" alt="">
                    </div>
                    <div class="col">
                        <img class="d-block m-auto footer-img" src="../images/home/claro-musica-icon.svg" alt="">
                    </div>
                </div>
                <div class="d-flex padding-bottom-footer">
                    <div class="col">
                        <img class="d-block m-auto small-footer-img" src="../images/home/sanborns-icon.svg" alt="">
                    </div>
                    <div class="col">
                        <img class="d-block m-auto small-footer-img" src="../images/home/soumaya-icon.svg" alt="">
                    </div>
                    <div class="col">
                        <img class="d-block m-auto small-footer-img" src="../images/home/claro-shop-icon.svg" alt="">
                    </div>
                    <div class="col">
                        <img class="d-block m-auto small-footer-img" src="../images/home/fundacion-icon.svg" alt="">
                    </div>
                </div>
            </div>
        </div>
        <!--Términos y condiciones para desktop-->
        <div class="terms-conditions-desktop">
            <div class="row aviso-border d-flex align-items-end">
                <div class="col-12 col-md-10 col-lg-6 col-xl-6">
                    <div class="logo-footer d-xl-inline-block">
                        <div class="p-xl-3 position-relative border-t-gray border-b-gray border-r-gray border-l-gray">
                            <img src="./images/pencil.svg" alt="" class="a-pencil-icon d-none d-xl-block">
                            <img src="../images/home/claro-grey.svg" alt="" class="claro-grey-img" />
                        </div>
                    </div>
                    <div class="p-xl-3 border-t-gray border-b-gray border-l-gray border-r-gray position-relative">
                        <img src="./images/pencil.svg" alt="" class="a-pencil-icon d-none d-xl-block">
                        <p class="text-footer-regular text-footer-basic">
                            @2019 Claro tv. Todos los derechos reservados
                        </p>
                    </div>

                    <div class="d-flex mb-xl-4">
                        <div class="p-xl-3 border-b-gray border-l-gray border-r-gray position-relative">
                            <img src="./images/pencil.svg" alt="" class="a-pencil-icon d-none d-xl-block">
                            <a href="terminos.html">
                                <p class="aviso-priv-blue">Términos y condiciones</p>
                            </a>
                        </div>
                        <div class="p-xl-3 border-b-gray border-r-gray position-relative">
                            <img src="./images/pencil.svg" alt="" class="a-pencil-icon d-none d-xl-block">
                            <a href="#">
                                <p class="aviso-priv-blue">Aviso de privacidad</p>
                            </a>
                        </div>
                    </div>
                </div>

            </div>

            <!--Div logos de compañías-->

            <!--Versión desktop de divs de logos de las compañías-->
            <div class="footer-companies-desktop">
                <div class="d-flex padding-bottom-footer col-lg-12 col-xl-12 m-auto justify-content-between align-items-center">
                    <div class="col footer-border-right position-relative">
                        <img src="./images/pencil.svg" alt="" class="a-pencil-icon d-none d-xl-block">
                        <img class="d-block m-auto small-footer-img" src="../images/home/sanborns-icon.svg" alt="" />
                    </div>
                    <div class="col footer-border-right position-relative">
                        <img src="./images/pencil.svg" alt="" class="a-pencil-icon d-none d-xl-block">
                        <img class="d-block m-auto footer-img" src="../images/home/claro-musica-icon.svg" alt="" />
                    </div>
                    <div class="col footer-border-right position-relative">
                        <img src="./images/pencil.svg" alt="" class="a-pencil-icon d-none d-xl-block">
                        <img class="d-block m-auto footer-img" src="../images/home/claro-video-icon.svg" alt="" />
                    </div>

                    <div class="col footer-border-right position-relative">
                        <img src="./images/pencil.svg" alt="" class="a-pencil-icon d-none d-xl-block">
                        <img class="d-block m-auto footer-img" src="../images/home/fast-icon.svg" alt="" />
                    </div>
                    <div class="col footer-border-right position-relative">
                        <img src="./images/pencil.svg" alt="" class="a-pencil-icon d-none d-xl-block">
                        <img class="d-block m-auto small-footer-img" src="../images/home/soumaya-icon.svg" alt="" />
                    </div>
                    <div class="col footer-border-right position-relative">
                        <img src="./images/pencil.svg" alt="" class="a-pencil-icon d-none d-xl-block">
                        <img class="d-block m-auto small-footer-img" src="../images/home/claro-shop-icon.svg" alt="" />
                    </div>
                    <div class="col position-relative">
                        <img src="./images/pencil.svg" alt="" class="a-pencil-icon d-none d-xl-block">
                        <img class="d-block m-auto small-footer-img" src="../images/home/fundacion-icon.svg" alt="" />
                    </div>
                </div>
            </div>
            <!--Fin div logos de compañías-->
        </div>
</footer>